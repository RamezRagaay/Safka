...
const ProductCard = ({ product , offer=undefined}) => {
  const [isHovered, setIsHovered] = useState(false)
  console.log(product);
    
  return (
    <div className='relative '>
      {offer && <div className='ribbon bg-red-600'>{offer}%</div>}
      <Tooltip.Provider delayDuration={300}>
        <div 
          className='w-full xl:w-full h-[400px] border flex flex-col justify-between shadow-sm transition-all duration-300 hover:shadow-lg rounded-lg  scale-90'
          onMouseEnter={() => setIsHovered(true)} 
          onMouseLeave={() => setIsHovered(false)}
          dir="rtl"
        >
          <div className='relative h-1/2 w-full border-b bg-white'>
